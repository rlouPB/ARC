        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                if(listViewResult.length > 0){
                    console.log('listViewResult')
                    console.log(listViewResult)

                    // set listViewResult attribute with response
                    component.set("v.listViewResult",listViewResult);

                    var developerName = component.get("v.currentListViewName")
                    if(developerName){
                        component.set("v.currentListViewName", listViewResult[listViewResult.findIndex(x=> x.developerName == developerName)].developerName);
                    }
                    else{
